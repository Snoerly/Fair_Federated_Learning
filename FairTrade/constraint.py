...
class ConstraintLoss(nn.Module):
    def __init__(self, n_class=2, alpha=1, p_norm=2):
        ...
        self.alpha = alpha if type(alpha) != torch.Tensor else alpha.to(self.device)
        self.p_norm = p_norm
...
def _safe_mean(t: torch.Tensor) -> torch.Tensor:
    # Avoid NaNs when a group has no samples; return zero in that case.
    if t.numel() == 0:
...
class DemographicParityLoss(ConstraintLoss):
    def __init__(self, sensitive_classes=[0, 1], alpha=1, p_norm=2):
        ...
        super().__init__(n_class=self.n_class, alpha=alpha, p_norm=p_norm)
