...
def main():
    ...
    parser.add_argument(
        "--epochs", type=int, default=10, help="Global rounds (E in den Plots)."
    )
    parser.add_argument(
        "--local_ep", type=int, default=10, help="Lokale Epochen pro Client."
    )
    parser.add_argument("--local_bs", type=int, default=10, help="Lokale Batchgröße.")
    parser.add_argument(
        "--frac",
        type=float,
        default=0.1,
    ...
    for i, cmd in enumerate(all_runs, 1):
        ...
        if res.returncode != 0:
            print(
                "  -> Lauf abgebrochen (return code {}). Stop.".format(res.returncode)
            )
            break
        else:
