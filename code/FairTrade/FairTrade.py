...
parser = argparse.ArgumentParser(
    description="pass the following arguments: dataset_name, number of clients, fairness notion, number of communication rounds."
)
...
# DNN model producing logits (use BCEWithLogitsLoss downstream)
def create_model(input_dim):
    model = nn.Sequential(
...
parser.add_argument(
    ...
    default=3,
    choices=[3],
    help="Number of clients. Currently only supports 3.",
)
parser.add_argument(
    "--dataset_name",
    type=str,
...
parser.add_argument(
    "--epochs", type=int, default=15, help="Client training epochs. Default is 15."
)
parser.add_argument(
...
clients_data, X_test, y_test, sex_list, column_names_list, ytest_potential = (
    load_dataset(url, dataset_name, num_clients, sensitive_feature, distribution_type)
)
...
def evaluate(alpha=100, lr=0.001, cost_false_negatives=5):
    ...
    for client_name in clients_data.keys():
        ...
        for epoch in range(epochs):
            ...
            logits = model1(X1).view(-1)
            weights = calculate_weights(y1, cost_false_negatives)
            # Use per-sample weights via the "weight" argument (not pos_weight)
            criterion = torch.nn.BCEWithLogitsLoss(weight=weights)
            fairness_loss = dp_loss(X1, logits, s1, y1_potential)
            fairness_loss = fairness_loss.to(device)
            loss = criterion(logits, y1) + fairness_loss
            loss.backward()
            optimizer1.step()
    ...
    with torch.no_grad():
        ...
        (
            ...
            acc,
            auc,
        ) = all_metrics(y_test.cpu(), probs.cpu())
    ...
    for i in range(mobo_optimization_rounds):  # number of rounds of mobo optimization
        ...
        acq_func = qExpectedHypervolumeImprovement(
            ...
            ref_point=torch.tensor(
                [0.001, 0.001]
            ),  # problem.ref_point.tolist(),  # use known reference point
        ...
        for i, m in enumerate(model.models):
            ...
            new_objectives = evaluate(
                candidate[0, 0].item(), candidate[0, 1].item()
            )  # evaluate(candidate.item())
...
if distribution_type == "random":
    if fairness_notion == "stat_parity":
        np.save(
            ...
            + "/"
            + str(num_clients)
            + "_bal_acc_stat_parity.npy",
            np.array(bal_acc_list),
        )
        np.save(
            destination + dataset_name + "/" + str(num_clients) + "_stat_parity.npy",
            np.array(fairness_notion_list),
        )
    else:
        np.save(
            destination + dataset_name + "/" + str(num_clients) + "_bal_acc_ate.npy",
            np.array(bal_acc_list),
        )
        np.save(
            destination + dataset_name + "/" + str(num_clients) + "_ate.npy",
            np.array(fairness_notion_list),
        )
else:
    if fairness_notion == "stat_parity":
        ...
        np.save(
            ...
            + "/"
            + str(num_clients)
            + "_attr_bal_acc_ate.npy",
            np.array(bal_acc_list),
        )
        np.save(
            destination + dataset_name + "/" + str(num_clients) + "_attr_ate.npy",
            np.array(fairness_notion_list),
        )
