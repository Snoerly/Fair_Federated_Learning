...
def find_potential_outcomes(H, observed_output, sensitive_feature):  # H is client_data
    ...
    potential_output = find_potential_outcomes_ordered(
        psm, observed_output, knn, sensitive_feature
    )
...
def find_potential_outcomes_ordered(psm, observed_output, knn, sensitive_feature):
    ...
    potential_output_female_dict = {
        female.index[i]: observed_output[idx]
        for i, idx in enumerate(neighbor_indexes_female[:, 0])
    }
    potential_output_male_dict = {
        male.index[i]: observed_output[idx]
        for i, idx in enumerate(neighbor_indexes_male[:, 0])
    }
    ...
    potential_output_ordered = [
        potential_output_dict[i] for i in sorted(potential_output_dict.keys())
    ]
...
def load_adult_age_country(url, sensitive_feature):
    ...
    mask = (
        ...
        | (data["native.country"] == "Cuba")
        | (data["native.country"] == "Jamaica")
        | (data["native.country"] == "Mexico")
        | (data["native.country"] == "Puerto-Rico")
        | (data["native.country"] == "Honduras")
        | (data["native.country"] == "Canada")
        | (data["native.country"] == "Haiti")
        | (data["native.country"] == "Dominican-Republic")
        | (data["native.country"] == "El-Salvador")
        | (data["native.country"] == " Guatemala")
        | (data["native.country"] == " Outlying-US(Guam-USVI-etc)")
        | (data["native.country"] == "Nicaragua")
        | (data["native.country"] == "Columbia")
        | (data["native.country"] == "Ecuador")
        | (data["native.country"] == "Peru")
        | (data["native.country"] == "Trinadad|Tobago")
    )
    df1 = data[mask]
    ...
    mask = (
        ...
        | (data["native.country"] == "?")
        | (data["native.country"] == "South")
        | (data["native.country"] == "Iran")
        | (data["native.country"] == "Philippines")
        | (data["native.country"] == "Cambodia")
        | (data["native.country"] == "Thailand")
        | (data["native.country"] == "Laos")
        | (data["native.country"] == "Taiwan")
        | (data["native.country"] == "China")
        | (data["native.country"] == "Japan")
        | (data["native.country"] == "Vietnam")
        | (data["native.country"] == "Hong")
    )
    df2 = data[mask]

    mask = (
        (data["native.country"] == "England")
        | (data["native.country"] == "Germany")
        | (data["native.country"] == "Italy")
        | (data["native.country"] == "Poland")
        | (data["native.country"] == "Portugal")
        | (data["native.country"] == "France")
        | (data["native.country"] == "Yugoslavia")
        | (data["native.country"] == "Scotland")
        | (data["native.country"] == "Greece")
        | (data["native.country"] == "Ireland")
        | (data["native.country"] == "Hungary")
        | (data["native.country"] == "Holand-Netherlands")
    )
    df3 = data[mask]
    ...
    y_potential_client1 = find_potential_outcomes(
        X_client1, y_client1, sensitive_feature
    )
    ...
    y_potential_client2 = find_potential_outcomes(
        X_client2, y_client2, sensitive_feature
    )
    ...
    y_potential_client3 = find_potential_outcomes(
        X_client3, y_client3, sensitive_feature
    )
    ...
    data_dict["client_1"] = {
        ...
        "y": y_client1,
        "s": s_client1,
        "y_pot": y_potential_client1,
    }
    data_dict["client_2"] = {
        "X": X_client2,
        "y": y_client2,
        "s": s_client2,
        "y_pot": y_potential_client2,
    }
    data_dict["client_3"] = {
        "X": X_client3,
        "y": y_client3,
        "s": s_client3,
        "y_pot": y_potential_client3,
    }
    # print("bismillah")
...
def load_dataset(url, dataset_name, num_clients, sensitive_feature, distribution_type):
    if dataset_name == "adult":
        if distribution_type == "random":
            ...
            data_dict, X_test, y_test, sex_list, column_names_list, ytest_potential = (
                load_adult_age(url, sensitive_feature)
            )
            return (
                data_dict,
                X_test,
        ...
        if distribution_type == "random":
            ...
            data_dict, X_test, y_test, sex_list, column_names_list, ytest_potential = (
                load_bank_age(url, sensitive_feature)
            )
            return (
                data_dict,
                X_test,
        ...
        if distribution_type == "random":
            ...
            data_dict, X_test, y_test, sex_list, column_names_list, ytest_potential = (
                load_default_age(url, sensitive_feature)
            )
            return (
                data_dict,
                X_test,
        ...
        if distribution_type == "random":
            ...
            data_dict, X_test, y_test, sex_list, column_names_list, ytest_potential = (
                load_law_income(url, sensitive_feature)
            )
            return (
                data_dict,
                X_test,
        ...
        if distribution_type == "random":
            ...
            data_dict, X_test, y_test, sex_list, column_names_list, ytest_potential = (
                load_kdd_age(url, sensitive_feature)
            )
            return (
                data_dict,
                X_test,
    ...
    if k == 0:
        ...
        for i in range(num_clients):
            if i == num_clients - 1:
                ...
                X_temp, X_client, y_temp, y_client = train_test_split(
                    X_temp, y_temp, test_size=1 / (num_clients - i), random_state=42
                )

            s_client = X_client[sensitive_feature]
            # compute potential outcomes
            y_potential_client = find_potential_outcomes(
                X_client, y_client, sensitive_feature
            )
