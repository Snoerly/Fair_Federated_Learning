...
class LocalUpdate:
    ...
    def train_val_test(
        self, dataset: Dataset, idxs: Sequence[int]
        ...
        def _loader(
            sub_idxs: Sequence[int], batch_size: int, shuffle: bool
        ) -> DataLoader:
            return DataLoader(
                DatasetSplit(dataset, sub_idxs), batch_size=batch_size, shuffle=shuffle
            )

        return (
            _loader(idxs_train, batch_size=max(1, self.args.local_bs), shuffle=True),
            _loader(
                idxs_val, batch_size=max(1, len(idxs_val) // 2 or 1), shuffle=False
            ),
            _loader(
                idxs_test, batch_size=max(1, len(idxs_test) // 2 or 1), shuffle=False
            ),
        )
...
def test_inference(
    args: Args, model: nn.Module, test_dataset: Dataset
) -> Tuple[float, float]:
    model.eval()
    loss, correct, total = 0.0, 0.0, 0.0
...
def run() -> None:
    ...
    csv_name = results_dir / (
        f"fedavg_{args.dataset}_{args.model}_users[{args.num_users}]_iid[{args.iid}]_C[{args.frac}]_"
    ...
    for epoch in range(args.epochs):
        ...
        for user in selected_users:
            local = LocalUpdate(
                args=args, dataset=train_dataset, idxs=user_groups[user]
            )
        ...
        # Fairness metrics (only if groups are present)
